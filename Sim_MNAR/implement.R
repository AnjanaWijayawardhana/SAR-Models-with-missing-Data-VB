...
mcmc.chains.T<-extract(fit_SEM_MNAR_HMC_thetayu)
...
df <- data.frame(
  ...
  Value = c(mcmc.chains.T$beta[,1], yu.theta.sample[,(nu+1)],
...
df <- data.frame(
  Group = rep(name.list, each = sample.size),
  Value = c(mcmc.chains.T$beta[,4], yu.theta.sample[,(nu+4)],
...
hmc.sigma2y.smple<-exp(mcmc.chains.T$gama)
...
hmc.rho.smple<-((exp(mcmc.chains.T$lamda) -1)/(exp(mcmc.chains.T$lamda) +1))
...
df <- data.frame(
  Group = rep(name.list, each = sample.size),
  Value = c(mcmc.chains.T$psi[,1], yu.theta.sample[,(nu+14)],
...
df <- data.frame(
  Group = rep(name.list, each = sample.size),
  Value = c(mcmc.chains.T$psi[,3], yu.theta.sample[,(nu+16)],
...
data <- data.frame(
  x = apply(mcmc.chains.T$yu,2,mean),  # Replace with your x values
...
data <- data.frame(
  x = apply(mcmc.chains.T$yu,2,sd),  # Replace with your x values
...
plot(apply(mcmc.chains.T$yu,2,sd),apply((yu_Aug_NoB$yugyom.chain)[(N1/2):N1,],2,sd))
# plot(apply(mcmc.chains.T$yu,2,sd),apply((yu_Aug_all$yugyom.chain)[5000:10000,],2,sd))
...
mcmc_array <- as.array(fit_SEM_MNAR_HMC_thetayu)
mcmc_matrix <- as.matrix(fit_SEM_MNAR_HMC_thetayu)
# head(mcmc_matrix)
beta0<-mcmc_matrix[,"beta[1]"]
beta1<-mcmc_matrix[,"beta[2]"]
gama<-mcmc_matrix[,"gama"]
sigma2y<-exp(gama)
lamda<-mcmc_matrix[,"lamda"]
