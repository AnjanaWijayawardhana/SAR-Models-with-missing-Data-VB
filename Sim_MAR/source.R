...
make.SATN.data<-function(x,N,w,y,no){
  ...
  x<-rbind(xs,xu) # create full (observe
  ...
  rho=0.3
  Min<-solve(I - rho * wpluwt + rho * rho * wtw)
  #priors
  p=ncol(x)
  ...
  stan.data.list<-list(n=length(y),no=ns,nu=nu,xo=xs,xu=xu,yo=ys,x=x,
                       p=ncol(x),w=w,I=I,wpluwt=wpluwt,Min=Min,
                       wtw=wtw, sigma2_beta=sigma2_beta,mu_beta=mu_beta)
  
  return(stan.data.list=stan.data.list)
  
  
} # STAN data list creater
...
post.sample.generate<-function(yo,x,w,theta){
  ...
  sigma_ugo<-sigma2y*solve(cholMuu,Diagonal(n-no)) # efficient cal of con var
